...
eventmodule.eventsCalendar = (ev,callback) =>{
  if(connection)
  {
    ...
    if(ev.id_mascotas==20 || ev.id_mascotas=='20')
    {
      ...
      var sql = 'SELECT  mascotas.*,events_masc.id_mascotas, mascotas.nombre as titls ,start, end,YEAR(start) as year, MONTH(start)-1 as month, DAY(start) as date FROM events_masc, mascotas WHERE events_masc.id_mascotas = mascotas.id_mascotas AND MONTH(start) = ? AND YEAR(start) = ? and id_servicios = ?'
    }
    else
    {
